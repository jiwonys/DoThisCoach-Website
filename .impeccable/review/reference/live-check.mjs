...
const origin='https://dothiscoach.com';const out='.impeccable/review/reference';
...
try {
 for(const width of [390,1440]) {
  ...
  await p.evaluate(async()=>{await document.fonts.ready;await Promise.all([...document.images].map(i=>{i.loading='eager';return i.decode()}));});
  assert(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
  assert.equal(await p.locator('img').count(),3);
  assert.equal(await p.locator('[data-download]').first().getAttribute('href'),'/app/');
  assert.equal(await p.locator('body').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(9, 14, 12)');
