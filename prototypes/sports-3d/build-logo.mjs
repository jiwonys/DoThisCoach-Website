...
await sharp(texture,{raw:{width:info.width,height:info.height,channels:4}}).extract({left:minX,top:minY,width,height}).png().toFile(resolve(import.meta.dirname,'logo-face.png'));
await writeFile(resolve(import.meta.dirname,'logo-shape.json'),JSON.stringify({source:'assets/dothis-logo.png',method:'Color-isolated contour extraction; original face pixels retained',width,height,contours:normalized}));
