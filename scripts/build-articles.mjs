...
// Stylesheets are served from GitHub Pages with `cache-control: max-age=600`
// and no hash in the filename, so a published style change kept looking
// undeployed to anyone holding a warm cache. Stamping the content hash onto
// the href makes each change a new URL, which the browser cannot serve stale.
const assetHashes = new Map();
const assetVersion = (relPath) => {
  if (!assetHashes.has(relPath)) {
    const contents = fs.readFileSync(path.join(root, relPath));
    assetHashes.set(relPath, crypto.createHash("sha1").update(contents).digest("hex").slice(0, 8));
  }
...
if (!catalogText) {
  ...
  // Advisory, not a gate. Workouts in the app are generated by the model, which
  // routinely produces exercises outside the catalog and handles them fine; a
  // catalog hit only buys better measurement metadata and guide content. Failing
  // the whole site build over an article naming a movement the catalog has not
  // caught up with was stopping useful writing for no reader benefit.
  const missing = [...exerciseNames].filter((name) => !catalogText.includes(`- ${name.toLowerCase()} (`));
  if (missing.length) {
    console.warn(`Article exercises not found in the DoThis catalog (guides and measurement metadata will be inferred rather than matched):\n- ${missing.join("\n- ")}`);
  }
}
...
<head>
  ...
  <link rel="icon" href="/favicon.ico" sizes="any" /><link rel="icon" type="image/png" sizes="96x96" href="/assets/icons/icon-96.png" /><link rel="apple-touch-icon" href="/apple-touch-icon.png" /><link rel="stylesheet" href="../../styles.css?v=${cssVersion()}"/><link rel="stylesheet" href="../article.css?v=${cssVersion()}"/><link rel="alternate" type="application/rss+xml" title="DoThis Training Library" href="../../feed.xml"/>
